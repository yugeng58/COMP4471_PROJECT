...
\author{ZhuFeiHao \quad 20949355\\
The Hong Kong University of Science and Technology\\
{\tt\small fhzhu@connect.ust.hk}\\
{\tt\small https://github.com/yugeng58/COMP4471\_PROJECT}}
...
detailed failure analysis identifying the primary root cause: frozen CLIP ViT
embeddings, trained for semantic image-text alignment, lack the spatial
configuration and dense correspondence information necessary for pixel-level
frame rendering. Contributing factors---loss functions that reward near-identity
outputs and conditioning mechanisms unable to express spatial motion---are
understood as consequences of this information bottleneck. Despite the negative
result, our analysis clarifies the requirements for embedding-conditioned video
...
Recent large-scale diffusion models such as Stable Video
Diffusion~\cite{blat2023} and Wan~\cite{wan2025} have demonstrated impressive
results by inflating image diffusion architectures into the temporal domain.
However, these end-to-end approaches incur substantial computational costs
and remain prone to motion flickering and identity drift.

Inspired by the JEPA framework~\cite{lecun2022}, we investigate an alternative: decoupling
...
model in a compact, abstract embedding space than in raw pixels. In an
ideal scenario, the Flow Model would learn high-level action dynamics (``a
walking person alternates legs''), and the Renderer would faithfully
translate these semantic blueprints into frames.

A central motivation for using frozen pre-trained encoders (CLIP ViT-B/32)
was to investigate whether their rich visual-semantic priors could reduce
training costs. Large-scale video generation models typically require
enormous datasets and compute budgets; if a frozen pre-trained
representation already captures meaningful visual structure, a lightweight
decoder might be sufficient to render motion within that representation
space. This is the core hypothesis we set out to test: \textbf{can frozen
pre-trained visual priors serve as an effective backbone for low-cost
...
near-identical outputs regardless of input conditioning. The primary root
cause, as our analysis will show, is that frozen CLIP ViT embeddings---trained
for semantic alignment rather than spatial correspondence---lack the
information content necessary for pixel-level rendering, a bottleneck that
no conditioning architecture or loss function could overcome.
...
Early video generation methods employed recurrent
networks~\cite{srivastava2015} or generative adversarial
networks~\cite{vondrick2016}. More recently, diffusion-based approaches have
become standard. Stable Video Diffusion~\cite{blat2023} fine-tunes a
pre-trained image diffusion model on video data by adding temporal layers.
Wan~\cite{wan2025} follows a similar approach at larger scale. These models generally operate in a VAE latent
...
representation space rather than input space. V-JEPA~\cite{bardes2024} applies this
...
the trajectory of CLIP embeddings~\cite{radford2021} rather than raw pixels. Where our work
...
information. FiLM~\cite{perez2018} applies channel-wise affine transformations
...
video prediction. Cross-attention, popularized by the
Transformer~\cite{vaswani2017} architecture, allows each spatial position in a feature map to attend to a
...
prediction followed by warping. Methods such as RAFT~\cite{teed2020} achieve
state-of-the-art optical flow estimation through iterative refinement,
while simpler approaches use Farneback optical flow as a supervision
...
controlled settings such as video game environments~\cite{oh2015} and
simplified human motion benchmarks. Text-to-video models such as Make-A-Video~\cite{singer2022} and
Imagen Video~\cite{ho2022} use large language model embeddings for
open-vocabulary conditioning at scale. Our setting is intermediate: we use closed-set
...
We use the HMDB51 dataset~\cite{kuehne2011}, containing 6,754 video clips across 51
...
25--30 fps. All available videos (6,754) are used for training; metrics reported in
this paper are computed on the training set, as our investigation focused
on training dynamics and conditioning sensitivity rather than
...
The pipeline is as follows: (1) a frozen CLIP ViT encodes the input image
$I_0$ to $E_0$ and extracts spatial patch tokens; (2) a frozen CLIP text
encoder produces the label embedding $L$; (3) the Flow Model---a causal
Transformer---predicts the embedding trajectory $(\hat{E}_1, \dots,
\hat{E}_n)$ from $(L, E_0)$ using shifted teacher forcing; (4) the
Renderer---a UNet with FiLM and cross-attention conditioning---produces
each frame $I_t$ from $I_{t-1}$, conditioned on $\hat{E}_t$, $L$, and
patch tokens. The two models are trained separately.
...
We initially attempted a continuous flow matching formulation: learning a
velocity field $v_\theta(E_t, t, L)$ that transports $E_0$ to $E_n$ through
the embedding space. This approach produced unstable trajectories and poor
convergence, which motivated our switch to a discrete autoregressive
formulation.

Our final Flow Model is a 4-layer causal Transformer encoder (12.9M
parameters) with 8 attention heads and feedforward dimension 2048. The
input sequence is $[L, E_0, E_1, \dots, E_{n-1}]$, and the model is
trained with shifted teacher forcing: position $t$ predicts $\hat{E}_{t}$
from $(L, E_{0, \dots, t-1})$.
...
The Renderer is a UNet~\cite{ronneberger2015} (4 down/up blocks,
40--43M parameters depending on variant) that takes $I_{t-1}$ as input and produces an output conditioned
...
combining them via coarse-to-fine refinement (similar to
RAFT~\cite{teed2020}). However,
...
We also report LPIPS~\cite{zhang2018} for perceptual quality assessment.
\end{itemize}
...
We identify the following contributing factors, ordered from most
fundamental to most specific.

\subsection{Primary Cause: Frozen CLIP Embeddings Are Unsuitable for This Task}

We believe this is the root cause from which all other issues derive.
Several lines of evidence point specifically to the frozen pre-trained
ViT embeddings---rather than the Transformer architecture or the decoupled
pipeline design---as the bottleneck.

First, purely autoregressive Transformer-based video generation has been
demonstrated successfully in prior work (e.g., VideoGPT, TATS, and more
recent video tokenizers combined with autoregressive transformers). These
models generate videos token-by-token or frame-by-frame, proving that the
autoregressive Transformer paradigm is not inherently flawed for video
generation. The key difference is that these models operate on
\emph{learned} discrete tokens from a VQ-VAE trained jointly for
reconstruction, whereas our Flow Model operates on \emph{frozen} CLIP
embeddings that were never optimized for the rendering task.

Second, CLIP ViT-B/32 is trained with a contrastive image-text matching
objective: given an image and a text description, maximize the cosine
similarity of their embeddings. This objective encourages the model to
extract \emph{semantic content}---``what is in this image''---and
compresses it into a compact 512-dimensional vector. What CLIP's training
objective does \emph{not} require is preserving the kind of information
needed for frame-to-frame rendering:

\begin{itemize}
    \item \textbf{Spatial configuration:} The precise 2D coordinates of
    body parts, objects, and their relative positions.
    \item \textbf{Pose and articulation:} The joint angles of limbs, the
    orientation of a head, the bend of a knee.
    \item \textbf{Dense correspondence:} Which pixel in frame $t$
    corresponds to which pixel in frame $t+1$.
\end{itemize}

A CLIP embedding can tell you ``this is a person walking'' with high
confidence, but it cannot tell you ``the right leg is currently forward
at a 30-degree angle, positioned at pixel coordinates (120, 180).'' The
latter is precisely the information a Renderer needs to predict what the
next frame should look like.

This explains the asymmetry in our results: the Flow Model succeeded
because predicting an embedding trajectory only requires matching the
statistical distribution of CLIP embeddings---a task CLIP embeddings
contain sufficient information for. The Renderer failed because decoding
an embedding back to a specific spatial configuration of pixels requires
information that was discarded during CLIP's encoding process.

Empirically, we confirmed this information bottleneck: adjacent frames of
the same video have CLIP embeddings with cosine similarity $\approx 0.976$
($\text{1-cos} \approx 0.024$). This tiny difference must encode the entire
motion transformation from one frame to the next---a compression ratio far
exceeding what any decoder architecture can plausibly invert.

\subsection{Contributing Factor: Loss Function Misalignment}
All our loss functions (L1~\cite{isola2017} on residuals, L1 on reconstruction after
warping) have an optimal trivial solution near the identity transformation:
$\Delta I_t \approx 0$ and $\text{flow} \approx 0$. When the conditioning
signal carries almost no usable information (as argued above), identity
mapping becomes the rational optimum.

\subsection{Contributing Factor: FiLM Cannot Express Spatial Guidance}
Feature-wise Linear Modulation applies channel-wise scaling and shifting
uniform across all spatial positions. Even if the embedding \emph{did}
contain spatial information, FiLM lacks the mechanism to route it to the
correct spatial location. Cross-attention to CLIP patch tokens partially
addresses this routing problem, but the patch tokens themselves still lack
dense correspondence information due to the primary cause above.
...
Our analysis suggests the primary cause is a fundamental information
bottleneck: CLIP ViT-B/32 embeddings are trained for semantic image-text
alignment and do not preserve the spatial configuration, pose, and
correspondence information necessary for pixel-level frame rendering.
Adjacent frames differ by less than 3\% in CLIP embedding cosine distance,
an information compression ratio that is implausible for any decoder
architecture to invert. All other observed problems---loss function
collapse to identity, FiLM's inability to express spatial motion---can
be understood as consequences of this root cause.

Returning to our original research question---whether frozen pre-trained
visual priors can serve as an effective backbone for low-cost I2V
generation---our results suggest a nuanced answer. The priors \emph{do}
carry useful semantic information: our Flow Model successfully learned
action-conditioned embedding trajectories with 82.5\% of ground-truth
motion variation, and the embedding space cleanly separates different
action classes (inter-class 1-cos $\approx$ 0.44 vs. intra-class $\approx$
0.34). However, semantic priors alone are insufficient for the
\emph{rendering} stage, which requires fine-grained spatial information
that CLIP's training objective discards. The lesson is not that pre-trained
priors are useless for video generation, but that the choice of
pre-trained representation must be matched to \emph{both} stages of a
decoupled pipeline---the prediction stage needs semantic structure, while
the rendering stage needs spatial correspondence.

\textbf{Future directions} suggested by this analysis include: (1)
replacing CLIP with a dense visual representation trained for spatial
correspondence, such as DINOv2 patch features, Stable Diffusion VAE
latents, or self-supervised optical flow features; (2) training the Flow
Model and Renderer jointly end-to-end, so the embedding space can adapt to
the specific needs of rendering rather than being constrained by a frozen
pre-trained encoder; (3) adopting losses that explicitly penalize lack of
motion, such as adversarial or perceptual losses, to break the near-identity
local minimum; (4) incorporating explicit motion representations such as
optical flow or keypoint trajectories as intermediate supervision targets
rather than relying solely on the embedding to carry all motion information.

{
    ...
    \begin{thebibliography}{99}
        ...
        \bibitem{wan2025} A.~Wang, B.~Ai, B.~Wen, et al. (Team Wan, Alibaba).
``Wan: Open and Advanced Large-Scale Video Generative Models.''
\textit{arXiv:2503.20314}, 2025.
        ...
        \bibitem{teed2020} Z.~Teed, J.~Deng.
