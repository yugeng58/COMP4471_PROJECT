...
\begin{center}
\ttfamily
[Input Image $I_0$] $\rightarrow$ [CLIP ViT] $\rightarrow$ [$E_0$] \\
{[Action Label]} $\rightarrow$ [CLIP Text] $\rightarrow$ [$L$] \\[4pt]
[$L$, $E_0$] $\rightarrow$ [\textbf{Flow Model}: Causal Transformer] $\rightarrow$ [$\hat{E}_1, \dots, \hat{E}_n$] \\[4pt]
[$I_{t-1}$, $\hat{E}_t$, $L$, patches] $\rightarrow$ [\textbf{Renderer}: FiLM + Cross-Attn UNet] $\rightarrow$ [$I_t$]
\end{center}
\smallskip
The Flow Model and Renderer are trained separately. At inference, the Flow
Model generates the embedding trajectory autoregressively; the Renderer
produces each frame conditioned on the predicted embedding.
\vspace{0.2cm}
\end{minipage}}
\caption{Overall pipeline. Frozen CLIP encoders produce image and label
embeddings. The Flow Model predicts the embedding trajectory. The Renderer
produces each output frame conditioned on the predicted embedding and the
previous frame.}
\label{fig:overview}
\end{figure*}
...
\subsection{Primary Cause: CLIP Embeddings Lack the Necessary Information}

We believe this is the root cause from which all other issues derive.
CLIP ViT-B/32 is trained with a contrastive image-text matching objective:
given an image and a text description, maximize the cosine similarity of
their embeddings. This objective encourages the model to extract
\emph{semantic content}---``what is in this image''---and compresses it
into a compact 512-dimensional vector.

What CLIP's training objective does \emph{not} require is preserving the
kind of information needed for frame-to-frame rendering:
